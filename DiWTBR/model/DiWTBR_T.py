...
class base_resnet(nn.Module):
    def __init__(self, basic_ch=64):
        ...
        self.model = ResNet(layers = [2,2,2,2])
        self.groupconv1 = nn.Conv2d(64, basic_ch, kernel_size=3, groups=32,padding=1)
        self.groupconv2 = nn.Conv2d(256, basic_ch*2, kernel_size=3, groups=32,padding=1)
        self.groupconv3 = nn.Conv2d(512, basic_ch*4, kernel_size=3, groups=32,padding=1)
        self.groupconv4 = nn.Conv2d(1024, basic_ch*8, kernel_size=3, groups=32,padding=1)
...
class DiWTBR_T(nn.Module):
    def __init__(self, basic_ch=32):
        super(DiWTBR_T, self).__init__()
        self.basic_ch = basic_ch
        self.extract_feature = base_resnet(basic_ch)
